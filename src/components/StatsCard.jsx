...
const StatsCard = ({ 
 ...
 const colorConfig = {
  neutral: {
    bg: 'bg-gray-100 dark:bg-[rgba(255,255,255,.1)]', // light for neutral, dark for dark mode
      ...
      <div className="relative p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center justify-between w-full space-x-3">
            ...
            <div>
              <h3 className={`text-sm ${config.text} capitalize line-clamp-1 tracking-wide font-bold`}>
                {title}
              </h3>
              {subtitle && (
                <p className={`text-sm ${config.text} capitalize line-clamp-1  tracking-wide font-bold`}>
                  {subtitle}
                </p>
              )}
            </div>
    ...
    <Icon className="w-5 h-5 icon" />
       ...
       <motion.div
        ...
        <div className="absolute bottom-0 right-0 w-20 h-20 opacity-10">
          <div className={`w-full h-full ${config.bg} rounded-[15px] transform translate-x-8 translate-y-8`} />
        </div>
      </div>
    </motion.div>
  )

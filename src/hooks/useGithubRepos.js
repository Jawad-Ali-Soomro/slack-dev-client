...
export default function useGithubRepos({ autoFetch = true } = {}) {
  ...
  const isCacheValid = useMemo(() => {
    ...
    if (!cachedGithubUserId || cachedGithubUserId !== githubAccountId) return false
    return true
  }, [
    isGithubConnected,
    currentUserId,
    ...
    async (force = false) => {
      ...
      try {
        ...
        if (data?.profile?.id && String(data.profile.id) !== githubAccountId) {
          dispatch(clearGithubData())
          dispatch(setError("GitHub account mismatch. Please reconnect GitHub."))
          return
        }
  ...
  useEffect(() => {
    ...
    const hasStaleCache = Boolean(encryptedData || cachedUserId || cachedGithubUserId)
    const shouldClear =
      hasStaleCache &&
      (!currentUserId ||
        !isGithubConnected ||

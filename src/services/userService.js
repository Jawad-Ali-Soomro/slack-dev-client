...
export const userService = {
  ...
  updateStatus: async ({ availability, jobRole, statusMessage } = {}) => {
    const payload = {};
    if (availability !== undefined) payload.availability = availability;
    if (jobRole !== undefined) payload.jobRole = jobRole;
    if (statusMessage !== undefined) payload.statusMessage = statusMessage;
    const response = await axiosInstance.put(`${API_BASE}/status`, payload);
    return response.data;
  },
  ...
  updateUserVerification: async (userId, emailVerified) => {
    const response = await axiosInstance.put(
      `${API_BASE}/admin/${userId}/verification`,
      { emailVerified },
    );
    return response.data;
  },

...
const enhancedMeetingService = {
  ...
  getMeetings: async (filters = {}) => {
    try {
      ...
      if (filters.assignedBy)
        queryParams.append("assignedBy", filters.assignedBy);
      if (filters.page) queryParams.append("page", filters.page);
      if (filters.limit) queryParams.append("limit", filters.limit);

      const response = await axios.get(
        `/api/meetings?${queryParams.toString()}`,
      );
      return response.data;
    } catch (error) {
      throw error.response?.data || error.message;
  ...
  reassignMeeting: async (meetingId, assignedTo) => {
    try {
      const response = await axios.put(`/api/meetings/${meetingId}/reassign`, {
        assignedTo,
      });
      return response.data;
    } catch (error) {
  ...
  updateAttendees: async (meetingId, attendees) => {
    try {
      const response = await axios.put(`/api/meetings/${meetingId}/attendees`, {
        attendees,
      });
      return response.data;
    } catch (error) {

...
export const useCodeCollaboration = () => {
  ...
  if (!context) {
    throw new Error(
      "useCodeCollaboration must be used within a CodeCollaborationProvider",
    );
  }
  return context;
};
...
export const CodeCollaborationProvider = ({ children }) => {
  ...
  const loadPublicSessions = async (language = null) => {
    try {
      const response = await codeCollaborationService.getPublicSessions(
        1,
        20,
        language,
      );
  ...
  const createSession = async (sessionData) => {
    try {
      const response =
        await codeCollaborationService.createSession(sessionData);
  ...
  const updateCode = async (code, cursorPosition = null) => {
    ...
    try {
      ...
      await codeCollaborationService.updateCode(
        currentSession._id,
        code,
        cursorPosition,
      );
  ...
  const updateCursor = async (cursorPosition) => {
    ...
    try {
      ...
      await codeCollaborationService.updateCursor(
        currentSession._id,
        cursorPosition,
      );
    } catch (error) {
  ...
  const generateInviteCode = async (sessionId) => {
    try {
      const result =
        await codeCollaborationService.generateInviteCode(sessionId);
      return result.data;
    } catch (error) {
  ...
  const joinByInviteCode = async (inviteCode) => {
    try {
      ...
      const result =
        await codeCollaborationService.joinByInviteCode(inviteCode);
  ...
  const getSessionByInviteCode = async (inviteCode) => {
    try {
      const result =
        await codeCollaborationService.getSessionByInviteCode(inviteCode);
      return result.data;
    } catch (error) {
